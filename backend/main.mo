...
actor {
    ...
    public shared ({ caller }) func initializeAccessControl() : async () {
        ...
        switch (currentRole) {
            case (#guest) {
                // Only initialize if caller is still a guest (not yet authenticated)
                AccessControl.initialize(accessControlState, caller);
            };
            case _ {
                // Caller already has a role (#user or #admin), no action needed
    ...
    public query ({ caller }) func getPolishVoices() : async [Text] {
        if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
            Debug.trap("Unauthorized: Only users can access voice options");
        };
    ...
    // Font Preferences
    public query ({ caller }) func getAvailableFonts() : async [Text] {
        if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
            Debug.trap("Unauthorized: Only users can access font options");
        };
    ...
    // Reading Speeds
    public query ({ caller }) func getReadingSpeeds() : async [Text] {
        if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
            Debug.trap("Unauthorized: Only users can access reading speed options");
        };
    ...
    public query ({ caller }) func getHighlightColors() : async [Text] {
        if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
            Debug.trap("Unauthorized: Only users can access highlight color options");
        };
    ...
    public shared ({ caller }) func exportChapterAsDocx(chapterId : Text) : async ExportResult {
        if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
        ...
        switch (principalMap.get(chapters, caller)) {
                ...
                switch (textMap.get(userChapters, chapterId)) {
                    case (?chapter) {
                        ...
                        if (Text.size(response) == 0) {
                            return {
                                success = false;
                                message = "Export failed: Empty response";
                                data = null;
                            };
                        };
                        ...
                        if (encoded.size() == 0) {
                            return {
                                success = false;
                                message = "Export failed: Unable to convert response to Blob";
                                data = null;
                            };
                        } else {
                            var bytes = Array.init<Nat8>(encoded.size(), 0);
                            var i = 0;
                            label l for (b in encoded.vals()) {
                                bytes[i] := b;
                                i += 1;
                                if (i >= encoded.size()) break l;
                            };
                            return {
                                success = true;
                                message = "Export successful";
                                data = ?Array.freeze(bytes);
                            };
                        };
                    };
                    case null {
                        {
                            success = false;
                            message = "Export failed: Chapter not found";
                            data = null;
                        };
                    };
                };
            ...
            case null {
                {
                    success = false;
                    message = "Export failed: No chapters found for caller";
                    data = null;
                };
            };
        };
    ...
    public shared ({ caller }) func exportChapterAsPdf(chapterId : Text) : async ExportResult {
        if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
            Debug.trap("Unauthorized: Only users can export chapters");
        };
